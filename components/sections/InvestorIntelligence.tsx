...
const CALC_IMAGES: Record<string, string> = {
  "true-yield": "/images/intelligence-yield.jpg",
  "purchase-cost": "/images/services/acquisition.jpg",
  "payment-plan": "/images/services/strategy.jpg",
  "mortgage-estimate": "/images/intelligence-calculator.jpg",
};

const VIDEO_POSTERS = [
  "/images/morgan-hero.jpg",
  "/images/morgan-offer.jpg",
  "/images/morgan-portrait.jpg",
  "/images/morgan-walk-wave-poster.jpg",
];
...
export default function InvestorIntelligence() {
      ...
      node: (
        <Link href={`/insights/${article.slug}`} className="group flex h-full flex-col border border-silver bg-paper">
          ...
          <div className="card-pad flex flex-1 flex-col space-y-3">
            ...
            <p className="body-copy-sm">{article.excerpt}</p>
  ...
  for (const calc of liveCalculators) {
    if (calc.slug === "true-yield") {
      ...
      const img = CALC_IMAGES[calc.slug] ?? "/images/intelligence-calculator.jpg";
      rail.push({
        key: calc.slug,
        kind: "calculator",
        node: (
          <Link href={calc.href} className="group flex h-full flex-col border border-silver bg-paper">
            ...
            <div className="card-pad flex flex-1 flex-col space-y-3">
              ...
              <p className="body-copy-sm flex-1">{calc.summary}</p>
  ...
  rail.push({
    ...
    node: (
      <Link href="/the-real-numbers" className="group flex h-full flex-col border border-silver bg-paper">
        <div className="relative aspect-[16/10] overflow-hidden bg-surface">
          <Image
            src="/images/intelligence-market-data.jpg"
            alt="Dubai market skyline for The Real Numbers briefing"
            fill
            sizes="(min-width: 1024px) 34rem, 88vw"
            className="object-cover transition duration-700 group-hover:scale-[1.025]"
          />
    ...
    rail.push({
      ...
      node: (
        <VideoThumbnailCard
          ...
          posterSrc={VIDEO_POSTERS[index % VIDEO_POSTERS.length]}
        />
      ),
    });
  }
  ...
  return (
    <section id="intelligence" className="section-pad bg-paper" aria-label="Investor Intelligence">
      <div className="section-inner">
            ...
            >
              <div className="relative aspect-[16/10] bg-surface lg:aspect-auto lg:min-h-[22rem]">
                {featured.heroImage ? (
                  ...
                  <Image
                    src="/images/intelligence-yield.jpg"
                    alt=""
                    fill
                    sizes="(min-width: 1024px) 50vw, 100vw"
              ...
              <div className="flex flex-col justify-center space-y-4 p-6 sm:p-10 lg:p-12">
                <p className="eyebrow">Latest analysis</p>
                <h3 className="font-display text-2xl font-bold tracking-tight text-ink transition group-hover:text-maroon sm:text-4xl">
                  {featured.title}
                </h3>
                <p className="body-copy max-w-3xl sm:text-lg">{featured.excerpt}</p>
        ...
        <RevealOnScroll className="section-body">
          <div className="bg-maroon-dark px-6 py-10 text-white sm:px-12 sm:py-14">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-silver">True Yield</p>
            <h3 className="mt-4 font-display text-2xl font-bold tracking-tight sm:text-4xl">
