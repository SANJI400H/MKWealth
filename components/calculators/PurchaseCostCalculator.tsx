    ...
    <div className="grid gap-10 lg:grid-cols-2">
      <div className="space-y-4">
        ...
        <label className="block text-sm">
          ...
          <input className={`${fieldClass} mt-1`} value={dldPct} onChange={(e) => setDldPct(e.target.value)} />
        </label>
        <label className="block text-sm">
          <span className="text-ink-muted">Agency commission (%)</span>
          <input className={`${fieldClass} mt-1`} value={agencyPct} onChange={(e) => setAgencyPct(e.target.value)} />
        </label>
        <label className="block text-sm">
          <span className="text-ink-muted">Trustee / registration (AED)</span>
          <input className={`${fieldClass} mt-1`} value={trusteeFee} onChange={(e) => setTrusteeFee(e.target.value)} />
        </label>
        <label className="block text-sm">
          <span className="text-ink-muted">Admin / NOC / misc (AED)</span>
          <input className={`${fieldClass} mt-1`} value={adminFees} onChange={(e) => setAdminFees(e.target.value)} />
        </label>
        <label className="block text-sm">
          <span className="text-ink-muted">Furniture / fit-out (AED)</span>
          <input className={`${fieldClass} mt-1`} value={furniture} onChange={(e) => setFurniture(e.target.value)} />
        </label>
        <label className="block text-sm">
          <span className="text-ink-muted">Other acquisition (AED)</span>
          <input className={`${fieldClass} mt-1`} value={other} onChange={(e) => setOther(e.target.value)} />
        </label>
